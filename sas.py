...
class Human(pygame.sprite.Sprite):
    def __init__(self, pos_x, pos_y, board):
        ...
        self.rect = self.image.get_rect().move(self.cell_size * (self.x + self.y) + 6, SIZE / 2 - self.cell_size + self.cell_size * self.x - self.cell_size * self.y + 5)
...
if __name__ == '__main__':
    ...
    clock = pygame.time.Clock()
    running = True
    pole = Pole(0, 0, all_sprites)
    human = Human(13, 18, board)
    screen.fill((0, 0, 0))
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
